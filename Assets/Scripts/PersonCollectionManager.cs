...
/// </summary>
public class PersonCollectionManager : Singleton<PersonCollectionManager>
{
    [Tooltip("A collection of Placeable objects related to the person.")]
    public List<GameObject> personObjectPrefabs;
    ...
    public void GenerateItemsInWorld(List<GameObject> horizontalSurfaces, List<GameObject> verticalSurfaces)
    {
        ...
        foreach (GameObject personPrefab in personObjectPrefabs)
        {
            Placeable placeable = personPrefab.GetComponent<Placeable>();
            if (placeable.PlacementSurface == PlacementSurfaces.Horizontal)
            {
                horizontalObjects.Add(personPrefab);
            }
            else
            {
                verticalObjects.Add(personPrefab);
            }
        }

        if (horizontalObjects.Count > 0)
        {
            CreatePersonObjects(horizontalObjects, horizontalSurfaces, PlacementSurfaces.Horizontal);
        }

        if (verticalObjects.Count > 0)
        {
            CreatePersonObjects(verticalObjects, verticalSurfaces, PlacementSurfaces.Vertical);
        }
    }
    ...
    /// </summary>
    /// <param name="personObjects">Collection of prefab GameObjects that have the Placeable component.</param>
    ...
    private void CreatePersonObjects(List<GameObject> personObjects, List<GameObject> surfaces, PlacementSurfaces surfaceType)
    {
        ...
        surfaces.Sort((lhs, rhs) =>
       {
        ...
        for (int i = surfaces.Count - 1; i >= 0; i--)
        {
            ...
            Vector3 corner = col.ClosestPointOnBounds(headPos);
            Debug.Log("bork bork: " + Vector3.Distance(corner, headPos));

            if (Vector3.Distance(corner, headPos) < 0.2f)
            {
                surfaces.RemoveAt(i);
            }
        }

        foreach (GameObject item in personObjects)
        {
            int index = -1;
            ...
            if (index >= 0)
            {
                ...
                position = position + Camera.main.transform.forward.normalized;
            ...
            GameObject personObject = Instantiate(item, position, rotation) as GameObject;
            personObject.transform.parent = gameObject.transform;
            personObject.SendMessage("OnPlacementComplete");
        }
    }    
